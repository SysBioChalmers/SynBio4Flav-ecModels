...
delLimit     = 0.05;
step         = 0;
essential = readtable('essentialGenes.txt','Delimiter','\t');
essential = strtrim(essential.gene_id);
...
tempModel = setParam(tempModel,'lb',targetIndx,0); %growth coupled???? lb = 0 originally
...
tempModel = setParam(tempModel,'lb',growth_indx,0.01);
tempModel = setParam(tempModel,'obj',targetIndx,1);
[WTsol,~] = solveECmodel(tempModel,tempModel,'pFBA',minIndex);
